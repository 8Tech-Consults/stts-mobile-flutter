...
class FormQDSList extends StatefulWidget {
  String title = "My QDS forms";

  FormQDSList({Key? key}) : super(key: key);

  @override
  State<FormQDSList> createState() => FormQDSListState();
}

late CustomTheme customTheme;

class FormQDSListState extends State<FormQDSList> {
  ...
  void initState() {
    ...
    srFormController.fetchQdsForm(locator<UserController>().user?.id);
  }
            ...
            appBar: AppBar(
              ...
              actions: [
                InkWell(
                    onTap: () {
                      setState(() {
                        Utils.navigate_to(AppConfig.formQDSScreen, context);
                      });
                    },
                    child: Container(
            ...
            body: SafeArea(
              child: model.isLoading
                  ...
                  : RefreshIndicator(
                      ...
                      child: model.qdsModel == null && !model.isLoading
                          ? MyWidgets.my_empty_widget(context,
                              'You have not created any QDS form. Press the "+" button in top right conner to create one.')
                          : SrWidget(
                              item: model.qdsModel,
                            ),
                    ),
  ...
  Future<void> _do_refresh() async {
    is_loading = true;
    setState(() {});
    items = await QDSModel.get_items();
            ...
            onSelected: (x) {
              if (x == 0) {
                ...
                Utils.launchURL(AppConfig.dashboardUrl +
                    "/form-crop-declarations/${item.id}");
              } else if (x == 3) {
                Utils.navigate_to(AppConfig.detailsScreen, context,
                    data: item.get_details());
              }
  ...
  Future<void> do_delete(int id) async {
    is_loading = true;
    setState(() {});
    String raw = await Utils.http_post('forms/qds/delete', {'id': id});
